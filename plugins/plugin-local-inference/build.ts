...
const result = await Bun.build({
	// Entrypoints MUST start with "./". Without it, Bun.build treats the
	// path as ambiguous and mis-roots relative-import resolution for the
	// secondary entrypoints — `./dflash-*` imports reached via
	// src/services/index.ts then fail with "Could not resolve" on Linux/CI
	// while still working on macOS (oven-sh/bun#12734).
	entrypoints: [
		"./src/index.ts",
